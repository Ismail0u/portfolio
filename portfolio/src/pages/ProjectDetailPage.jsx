...
const Block = ({ title, children, delay }) => (
  <motion.div
    ...
    variants={fadeUp}
    custom={delay}
    className="py-8 border-b border-fg/10"
  >
    <h2 className="font-display font-semibold text-fg/40 text-xs uppercase tracking-widest mb-3">
      {title}
    </h2>
    {children}
  </motion.div>
...
const BulletList = ({ items }) => (
  <ul className="space-y-2">
    {items.map((item, i) => (
      <li key={i} className="flex items-start gap-2 text-sm text-fg/70 leading-relaxed">
  ...
  return (
    <div className="pt-28 pb-24 max-w-2xl">
      ...
      <motion.div initial="hidden" animate="visible" variants={fadeUp} custom={0.1} className="mt-6 brand-glow">
        <h1 className="font-display font-bold text-4xl sm:text-5xl text-fg">{project.title}</h1>
        <p className="mt-4 text-fg/60 leading-relaxed max-w-xl">
          {pickLang(project, 'description', lang)}
        </p>

        <div className="mt-4 flex flex-wrap gap-2">
          {project.tech.map((tech) => (
            <span key={tech} className="text-xs px-2.5 py-1 rounded-full bg-fg/5 text-fg/50">
              {tech}
            </span>
          ))}
        </div>
      ...
      {cs ? (
        <div className="mt-10">
          <Block title={t('projects.problem')} delay={0.05}>
            <p className="text-sm text-fg/70 leading-relaxed">{cs[`problem${suffix}`]}</p>
          </Block>
          <Block title={t('projects.solution')} delay={0.1}>
            <p className="text-sm text-fg/70 leading-relaxed">{cs[`solution${suffix}`]}</p>
          </Block>
          <Block title={t('projects.role')} delay={0.15}>
            <p className="text-sm text-fg/70 leading-relaxed">{cs[`role${suffix}`]}</p>
          </Block>
          ...
          {cs[`results${suffix}`] && (
            <Block title={t('projects.outcome')} delay={0.35}>
              <p className="text-sm text-fg/70 leading-relaxed">{cs[`results${suffix}`]}</p>
            </Block>
          )}
        </div>
      ) : (
        <p className="mt-10 text-sm text-fg/40">{t('projects.noCaseStudy')}</p>
      )}
    </div>
  );
}
